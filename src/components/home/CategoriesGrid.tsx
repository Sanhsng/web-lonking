          ...
          {displayCategories.map((category, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 w-full p-3 sm:p-5 flex flex-col gap-0 sm:gap-1">
                  <div className="bg-white/20 backdrop-blur-md w-8 h-8 sm:w-12 sm:h-12 rounded-full flex items-center justify-center mb-0.5 sm:mb-1 border border-white/30">
                    <Icon className="w-4 h-4 sm:w-6 sm:h-6 text-white" />
                  </div>
                  <h3 className="font-headline-md text-[14px] sm:text-[20px] md:text-[24px] text-white font-semibold leading-tight">
                    {category.title}
                  </h3>
                  <span className="text-white/80 font-label-sm text-[10px] sm:text-label-sm uppercase tracking-wider group-hover:text-secondary-fixed transition-colors flex items-center gap-1 font-semibold mt-0.5 sm:mt-1">
